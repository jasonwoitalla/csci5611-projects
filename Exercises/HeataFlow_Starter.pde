...
//    Update the code so that r actually resets the heat to its inital condition. [X]
// 2. Recall the heat equation says the amount of heat will change at a rate proportional 
//    to the negative of the laplacian, confirm that it is implemented correctly here. [X]
// 3. The laplacian of a function is defined by how fast the slope changes. Prove that
//    the variables "laplacian" and "laplacian_alt" compute the same value. [X]
...
void colorByTemp(float u){
  float input = constrain(u, 0.0, 1.0);
