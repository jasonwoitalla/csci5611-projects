...
class Camera
{
  Camera()
  {
    position      = new PVector(738, 143, 798); // initial position
    theta         = 0.37015617; // rotation around Y axis. Starts with forward direction as ( 0, 0, -1 )
    phi           = -0.15383558; // rotation around X axis. Starts with up direction as ( 0, 1, 0 )
    moveSpeed     = 50;
  ...
  void HandleKeyPressed()
  {
    ...
    if ( key == 'p' || key == 'P' ) println("Loc: " + position + " Theta: " + theta + " Phi: " + phi);
